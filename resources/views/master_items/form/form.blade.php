<form method="POST" enctype="multipart/form-data">
    ...
    <div class="form-group">
        ...
        @if (!empty($item->foto))
            <div class="mb-2">
                <img src="{{ asset('uploads/' . $item->foto) }}" alt="Foto" width="120" class="img-thumbnail">
            </div>
        @endif
    ...
    <div class="form-group">
        ...
        <select class="form-control" name="categories[]" multiple>
            @foreach ($categories as $kategori)
                <option value="{{ $kategori->id }}" @if ($method == 'edit' && $item->categories->contains($kategori->id)) selected @endif>
                    {{ $kategori->kode }} - {{ $kategori->nama }}
                </option>
            @endforeach
        </select>
        <small class="form-text text-muted">Pilih lebih dari satu kategori tekan Ctrl</small>
    </div>
